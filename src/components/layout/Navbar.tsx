...
export function Navbar() {
  ...
  const { user, loading, openLoginModal, openRegisterModal } = useAuth();
        ...
        <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          <Link href="/" className="flex items-center gap-2.5 group focus-ring rounded-lg">
            ...
            <span className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">
              Sudipto<span className="text-gradient"></span>
            </span>
          </Link>
            ...
            {!loading && (
              !user ? (
                <div className="hidden sm:flex items-center gap-2">
                  <button onClick={openLoginModal} className="text-sm font-medium text-slate-600 hover:text-brand-600 dark:text-slate-400 dark:hover:text-brand-400 px-3 py-2 rounded-lg transition-colors">
                    Login
                  </button>
                  <button onClick={openRegisterModal} className="bg-brand-600 hover:bg-brand-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
                    Register
                  </button>
                </div>
              ) : (
                <div className="hidden sm:flex items-center gap-2">
                  <button className="flex h-9 w-9 items-center justify-center rounded-xl transition-all duration-200 focus-ring bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-900 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-100">
